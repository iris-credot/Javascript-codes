...
if (longWord) {
  const replacementWord = 'stunning'; // You can choose any replacement word here
  const index = storyWords.indexOf(longWord);
  const newStory = storyWords.slice(0, index).join(' ') + ' ' + replacementWord + ' ' + storyWords.slice(index + 1).join(' ');
  
  
  console.log(newStory); // Output: It was a really stunning day. The sun was shining and the birds were singing.
} else {
  console.log("No word longer than 10 characters found.");
}
